...
<%@page import="ctu.edu.vn.dlab.service.RegistrationLabService"%>
<%@page import="ctu.edu.vn.dlab.model.RegistrationLabStudent"%>
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Đăng ký lớp thực hành</li>
                    </ol>
                </section>
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="box box-primary">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">Đăng ký lớp thực hành</h3>                                    
                                </div><!-- /.box-header -->
                                <div class="box-body table-responsive">
                                    <c:if test="${add_reglab_success != null}">
                                    <div class="alert alert-success alert-dismissable alert-top">
                                        <i class="fa fa-check"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <b>Alert!</b> ${add_reglab_success}
                                    </div>
                                    </c:if>
                                    
                                    <c:if test="${add_reglab_error != null}">
                                    <div class="alert alert-danger alert-dismissable alert-top">
                                        <i class="fa fa-ban"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <b>Alert!</b> ${add_reglab_error}
                                    </div>      
                                    </c:if>
                                    
                                    <c:if test="${add_reglab_limited != null}">
                                    <div class="alert alert-danger alert-dismissable alert-top">
                                        <i class="fa fa-ban"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <b>Alert!</b> ${add_reglab_limited}
                                    </div>      
                                    </c:if>
                                    ...
                                    <table id="example2" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <%
                                                ...
                                                List<RegistrationLabStudent> noRegistedList = (List<RegistrationLabStudent>) request.getAttribute("noRegistedList");
                                                if (noRegistedList != null && noRegistedList.size() > 0) {
                                                	int count = 0;
                                                	
                                                	for (RegistrationLabStudent reg : noRegistedList) {
                                                		count++;
                                            %>
                                            <tr>
                                                <td><%=count %></td>
                                                <td><%=reg.getName() %></td>
                                                <td><%=reg.getTotalRegisted() %>/<%=reg.getAmount() %></td>
                                                <td><%=reg.getIdCourse() %></td>
                                                <td><%=reg.getFullName() %></td>
                                                <td class="manager-lab-function-group">
                                                    <form action="${pageContext.request.contextPath}/student/register-lab" method="POST">
                                                        <input hidden="hidden" value="<%=reg.getIdLab() %>" name="idLab" />
